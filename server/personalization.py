...
def _truncate_corrections():
    """Keep only the last MAX_CORRECTIONS entries in the corrections file. Caller must hold _file_lock."""
...
def _truncate_history():
    """Keep only the last MAX_HISTORY entries. Caller must hold _file_lock."""
    if not HISTORY_FILE.exists():
        return
    with open(HISTORY_FILE, "r", encoding="utf-8") as f:
        lines = f.readlines()
